...
class CreateCustomMessages(object):
    """Create custom message content and send messages for suppliers and company"""
    def send_email(self, line, code, **kwargs):
        if not line.partner:
            return
        ctx = self.get_line_context(line)
        if getattr(settings, 'DEBUG'):
            ## Send to developer and not product owner
            email_address = getattr(settings, 'MASTER_EMAIL')
        else:
            email_address = getattr(settings, 'ADMIN_EMAIL_ADDRESS')
        try:
            ...
            Dispatcher().send_email_messages(email_address, messages)
    def get_line_context(self, line):
        ctx = {
            'line': line,
            'order': line.order,
