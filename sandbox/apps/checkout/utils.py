...
class CheckoutSessionData(object):
    ...
    def _get(self, namespace, key, default=None):
        """
        Return a value from within a namespaceri
        """
        self._check_namespace(namespace)
        print (namespace, 'namespace')
        print (self.SESSION_KEY, 'session key')
        print (self.request.session[self.SESSION_KEY], 'session in request')
        print (self.request.session[self.SESSION_KEY][namespace], 'namespace keys')
