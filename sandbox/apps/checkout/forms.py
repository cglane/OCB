...
class ShippingAddressForm(PhoneNumberMixin, AbstractAddressForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['state'].help_text = "Please use the state abbrevation. E.G 'SC'"
    ...
    def clean(self):
        super(ShippingAddressForm, self).clean()
        states_shipping = getattr(settings, 'SHIPPING_RATES')
        state_codes = [x for x in states_shipping]
        state = self.cleaned_data.get("state")
        if len(state) > 2:
            raise forms.ValidationError(
                "State cannot be longer than two characters."
            )
...
class GatewayForm(AuthenticationForm):
    ...
    def clean(self):
        print (self.cleaned_data, 'self clean')
