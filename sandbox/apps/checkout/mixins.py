...
class OrderPlacementMixin(CheckoutSessionMixin):
    ...
    supplier_code = 'ORDER_PLACED_SUPPLIER'
    bedswing_code = 'ORDER_PLACED_BEDSWING'
    ...
    def handle_successful_order(self, order):
        ...
        # Send confirmation message (normally an email) to buyer
        self.send_confirmation_message(order, self.communication_type_code)

        confirmation_emails = getattr(settings, 'CONFIRMATION_EMAIL_LIST')
        for email in confirmation_emails:
            self.send_confirmation_message_admin(order, self.bedswing_code, email)
            ##set timer to not overload SMTP
            time.sleep(2)

        self.send_confirmation_message_suppliers(order, self.supplier_code)
    ...
    def send_confirmation_message_suppliers(self, order, code, **kwargs):
        if order.lines:
            for line in order.lines.all():
                ctx = self.get_line_context(order, line)
                try:
                ...
                if messages and messages['body']:
                    ...
                    if getattr(settings, 'DEBUG'):
                        dispatcher.send_email_messages(getattr(settings,'MASTER_EMAIL'), messages)
                    else:
                        dispatcher.send_email_messages(line.partner.partner_email, messages)
                else:
                    logger.warning("Order #%s - no %s communication event type",
                                   order.number, code)
        return
    def send_confirmation_message_admin(self, order, code, email):
    ...
    def get_line_context(self, order, line):
        ctx = {
            'line': line,
            'order': order,
