...
import apps.shipping.methods as shipping_methods
...
class Repository(object):
    ...
    def get_available_shipping_methods(
        ...
        if not shipping_addr:
            methods = (shipping_methods.FixedPrice(0, 0),)
        else:
            shipping_rates = getattr(settings, 'SHIPPING_RATES')
            state = shipping_addr.state.upper()
            rate = shipping_rates.get(state, shipping_rates.get('CA'))
            methods = (shipping_methods.Standard(rate),)
        return methods
