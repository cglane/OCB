...
class Standard(Base):
    code = 'standard'
    name = 'Standard'
    def __init__(self, charge_excl_tax=None):
    ...
    def calculate(self, basket):
        ...
        mini_bedswings = product_types.count('Mini Bedswing')
        original_bedswings = product_types.count('Original Bedswing')
        total_bedswings = mini_bedswings + original_bedswings

        if total_bedswings:
            total_charge = self.charge_excl_tax * total_bedswings
        else:
            ### No Bedswings only cushions
            total_charge = self.charge_excl_tax
            total_charge = total_charge - deduction

        print (total_charge, 'total charge')

        return prices.Price(
            currency=basket.currency,
            excl_tax=total_charge,
            incl_tax=total_charge
        )
